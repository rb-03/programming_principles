...
class Map:
    ...
    def build_cell(self, row, col, cell):
        ...
        if ((row >= 0 and row < self.rows and col >= 0 and col < self._cols)==False): #condition: 
    ...
    def get_neighbours(self, row, col):
        ...
        for i in range(max(0, row - 1),min(row + 1, self._rows - 1)+1,1):
            for j in range(max(0, col - 1), min(col + 1, self._cols - 1)+1,1):
                if(i == row and j==col):
                    continue           
    ...
    def display(self):
        ...
        print("   ",end="")
        for i in range(0,self._cols,1): #column numbers displayed horizontally
            print("%d     "%i,end="")
        print("\n")
        for i in range(0,self._rows,1): #can be changed, use to print row numbers properly
            print("%d "%i,end="")
